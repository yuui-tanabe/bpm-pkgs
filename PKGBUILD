...
provides=('gcc-4.6')
depends=('glibc' 'binutils' 'gmp' 'mpfr' 'libmpc' 'ppl' 'isl15' 'cloog' 'elfutils' 'llvm33')
...
build() {
  ...
  ${srcdir}/${_basedir}/configure --prefix=/opt/gcc \
      --libdir=/opt/gcc/lib --libexecdir=/opt/gcc/lib \
...
package() {
  ...
  rm -rf ${pkgdir}/opt/gcc/share/

  # Move potentially conflicting stuff to version specific subdirectory
  $(ls "$pkgdir"/opt/gcc/lib/gcc/$CHOST/lib* 2&> /dev/null) && mv "$pkgdir"/opt/gcc/lib/gcc/$CHOST/lib* "$pkgdir/opt/gcc/lib/gcc/$CHOST/$pkgver/"
